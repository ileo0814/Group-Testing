...
#Checking sim.id
  if(df.sim$Protocol[df.sim$sim.id==sim.id]!="AT"){
    stop("The specified sim.id is not for AT")
  }


#Load (centered) true spatial random effects
#gamma.true <- read.table(paste0("C:/Users/Leo/OneDrive/huangleo Leo Huang/USC Files/2022 Spring (Current)/BIOS 890 Independent Study/results/gamma.true.txt"))
...
param.list <- list(G.mat.config = G.mat.seq,
                   n.size = n.size.seq,
                   p = p.seq,
                   N = N.seq)
...
p <- as.vector(exp(X%*%beta+G.mat%*%gamma)/(1+exp(X%*%beta+G.mat%*%gamma)))  
mean(p)
...
for(i in 1:n.pool){
  ...
  Z[i] <- rbinom(1,1,sens*Z.tilde[i]+(1-spec)*(1-Z.tilde[i]))
}
...
#save.image("C:/Users/Leo/OneDrive - University of South Carolina/R image/Array_testing/array052722.Rdata")
#load("C:/Users/Leo/OneDrive - University of South Carolina/R image/Array_testing/array052722.Rdata")
...
#Y.tilde MCMC samples
  ...
  Y.tilde.out <- rbind(Y.tilde.diff,Y.tilde.cover)
  Y.tilde.out
  
  plot(Y.tilde.true-colMeans(Y.tilde.pred),main="Residual plot - Y.tilde") 
